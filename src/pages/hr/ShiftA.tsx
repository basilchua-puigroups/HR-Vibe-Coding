...
const modules: Mod[] = [
  {
    label: 'Cages Tipped',
    path: '/human-resources/payroll/shift-a/cages-tipped',
...
export default function ShiftA() {
  const navigate = useNavigate();
  const { currentUser } = useAuth();

  if (!hasPerm(currentUser, 'viewShiftA')) return <NoPermission backPath="/human-resources/payroll" />;
    ...
    <>
      ...
      {visibleModules.length === 0 ? (
        <div className="panel" style={{ marginTop: 24, padding: 32, textAlign: 'center', color: 'var(--muted)' }}>
          <p>No Shift A modules available.</p>
        </div>
      ) : (
        <div className="module-grid" aria-label="Shift A modules" style={{ marginTop: 24 }}>
